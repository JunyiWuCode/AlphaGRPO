...
The prompt may contain stylistic tokens (e.g., "a photo of", "high-resolution photograph", "cinematic", "4k"), please do not output those phrase in tuples.
Do not missing the negation when analysing the entities.
...
# ### Extraction Rules:
# 1. **Style Removal:** The prompt may contain stylistic tokens (e.g., "a photo of", "high-resolution photograph", "cinematic", "4k"). Do NOT output these phases in tuples.
# 2. **Negation:** Do not miss the negation when analysing the entities (e.g., "no clouds" -> "sky | has | no clouds").
# 3. **Deduplication:** Do not generate the same tuples again.

# ### Implicit Subject Materialization (CRITICAL):
# While you must not hallucinate decorative objects, you **MUST infer the direct physical parts** required for an action or state to be physically valid:
# * **Action Agents:** If a prompt describes an action (e.g., "holding", "kicking"), identify the specific body part performing it.
#     * *Input:* "holding a sword" -> *Output:* `hand | holding | sword` (Instead of "person | holding | sword")
#     * *Input:* "kicking a ball" -> *Output:* `foot | kicking | ball`
# * **Sensory Visuals:** If an adjective implies a physical substance, extract it.
#     * *Input:* "hot coffee" -> *Output:* `steam | rising from | coffee`
...
# _TUPLE_PREAMBLE = """Task: Describe the scene from the input prompt using skill-specific tuples (subject | predicate | object).

# ### 1. Basic Rules
# * **No Style:** Ignore stylistic tokens (e.g., "cinematic", "4k", "masterpiece").
# * **No Negation Loss:** Explicitly capture negative constraints (e.g., "no clouds" -> "sky | has | no clouds").
# * **Deduplication:** Do not output identical tuples.

# ### 2. Implicit Entity Inference (CRITICAL)
# You MUST materialize entities that are **physically necessary** for the action/state to exist, even if not explicitly named in the prompt. Use the following categories:

# * **Anatomical Agents (Body Parts):**
#     For contact actions (holding, kicking, biting, kissing), identify the specific body part.
#     * *Example:* "holding a sword" -> `hand | holding | sword`
#     * *Example:* "winking" -> `eye | winking | null`

# * **Gravitational Supports (Surfaces):**
#     For static postures (sitting, leaning, lying), identify the supporting surface.
#     * *Example:* "sitting" -> `person | sitting on | seat/surface`
#     * *Example:* "leaning" -> `back | leaning against | wall/support`

# * **Instrumental Mediation (Tools):**
#     If an action strictly requires a tool to be visually identifiable, infer the generic tool.
#     * *Example:* "rowing" -> `hands | gripping | oars`
#     * *Example:* "typing" -> `fingers | typing on | keyboard`

# * **Visual Manifestation (Phenomena):**
#     Convert sensory adjectives into visible matter.
#     * *Example:* "hot tea" -> `steam | rising from | tea`
#     * *Example:* "windy" -> `hair/clothes | blowing in | wind`

# ### 3. Anti-Hallucination
# * Do NOT infer background elements that are optional (e.g., do NOT infer "running on grass" for "running").
# * Do NOT infer specific colors or styles unless specified.
...
### Extraction Rules
1. **Style Removal:** The prompt may contain stylistic tokens (e.g., "a photo of", "high-resolution photograph", "cinematic", "4k"), please do not output those phrase in tuples.
2. **Negation:** Do not miss the negation when analysing the entities (e.g., "no clouds" -> "sky has no clouds").
3. **Deduplication:** Do not generate same tuples again.
4. **Text & Quotes:** If a word or phrase is enclosed in single or double quotes (e.g., 'helmet', "Welcome"), you need to infer whether it is a physical object or text to be rendered. If it's the text, extract it as a text element (e.g., "text 'helmet'").

### Implicit Entity Inference (CRITICAL)
You MUST infer entities that are physically implied by the prompt to make the scene logically valid. Do not hallucinate optional background, but DO generate tuples for:
1. **Anatomical Agents:** If an action involves contact, identify the specific body part (e.g., "holding a sword" -> "hand holding sword").
2. **Gravitational Supports:** If a posture requires support, infer the surface (e.g., "sitting" -> "person sitting on surface").
3. **Instrumental Tools:** If an action strictly requires a tool, infer it (e.g., "rowing" -> "hands gripping oars").
4. **Visual Phenomena:** If an adjective implies a physical substance, infer it (e.g., "hot coffee" -> "steam rising from coffee", "reflection" -> "reflection on surface").

Do not generate tuples that are not explicitly described in the prompts EXCEPT for the implicit entities listed above.
...
# _TUPLE_PREAMBLE = """Task: given input prompts, describe each scene with skill-specific tuples.
# The prompt may contain stylistic tokens (e.g., "a photo of", "high-resolution photograph", "cinematic", "4k"), please do not output those phrase in tuples.
# Handling Negation: If the prompt uses negation (e.g., "no", "not", "without"), **include the negation word with the described content** in the tuple (e.g., return "not warming light", "no cars"). Do not output the content without its negation.
# Do not generate same tuples again. Do not generate tuples that are not explicitly described in the prompts. 
# output format: id | tuple
# """.strip()
...
def generate_with_in_context_examples(
	...
	if num_workers == 1:
		total_output = []
		for kwargs in tqdm.tqdm(total_kwargs,  disable=not verbose):
		...
		with Pool(num_workers) as p:
			...
			total_output = list(
				tqdm.tqdm(p.imap(generate_fn, total_inputs), total=len(total_inputs), disable=not verbose))
