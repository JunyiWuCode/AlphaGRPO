...
def get_config():
    ...
    # rescale clipped advantages to [-advantage_max, advantage_max] (None = no rescaling)
    train.advantage_max = None
    # the PPO clip range.
    train.clip_range = 1e-4
    # the fraction of timesteps to train on. if set to less than 1.0, the model will be trained on a subset of the
    # timesteps for each sample. this will speed up training but reduce the accuracy of policy gradient estimates.
    train.timestep_fraction = 1.0
    # image KL ratio (text KL uses text_beta)
    train.image_beta = 0.0
    # EMA KL weight (0 = disabled); uses a separate LoRA adapter for adaptive KL
    train.ema_beta = 0.0
    # KL weighting: 'Uniform' (plain MSE) or 'ELBO' (/ 2*std_dev_t^2)
    train.kl_weight = 'Uniform'
    train.kl_ema_weight = 'Uniform'
    # KL EMA adapter decay
    train.kl_ema_decay = 0.99
    train.kl_ema_decay_type = 'constant'  # 'constant' or 'linear'
    ...
    ###### Prompt Function ######
    # prompt dataset loader to use. See DATASET_MAP in alpha_grpo/dataset/__init__.py.
