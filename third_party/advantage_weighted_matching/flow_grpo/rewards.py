...
def unifiedreward_score_sglang(device):
    ...
    def _fn(images, prompts, metadata):
        # to handle Tensor type conversion
        if isinstance(images, torch.Tensor):
            images = (images * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
            images = images.transpose(0, 2, 3, 1)  # NCHW -> NHWC

        # to convert to PIL Image and adjust the size
        images = [Image.fromarray(image).resize((512, 512)) for image in images]

        # to execute asynchronous batch evaluation
...
def spectrareward_score(device):
    """Score image/prompt pairs with the shared SGLang SpectraReward client."""
    del device
    from rewards.sglang_spectrareward import SpectraRewardSGLangClient

    model_id = os.environ.get(
        "SPECTRAREWARD_MODEL_ID", "Qwen/Qwen3-VL-8B-Instruct"
    )
    url = os.environ.get("SPECTRAREWARD_URL")
    if not url:
        raise ValueError("The SD3 AWM runner requires SPECTRAREWARD_URL")
    scorer = SpectraRewardSGLangClient(
        ...
        exclude_eos=os.environ.get("SPECTRAREWARD_EXCLUDE_EOS", "1").lower()
        not in {"0", "false", "no"},
    ...
    def _fn(images, prompts, metadata):
        ...
        return scorer(pil_images, list(prompts)), {}
