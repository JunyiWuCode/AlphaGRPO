...
class JsonlPromptDataset(Dataset):
    def __init__(self, dataset, split="train"):
        self.file_path = (
            dataset if os.path.isfile(dataset) else os.path.join(dataset, f"{split}.jsonl")
        )
        with open(self.file_path, "r", encoding="utf-8") as handle:
            self.metadatas = [json.loads(line) for line in handle if line.strip()]
        self.prompts = [item["prompt"] for item in self.metadatas]
    ...
    collate_fn = staticmethod(GenevalPromptDataset.collate_fn)
...
def save_ckpt(save_dir, transformer, global_step, epoch, accelerator, ema, transformer_trainable_parameters, config):
...
def main(_):
    ...
    if config.resume_from:
        ...
        if not os.path.basename(config.resume_from).startswith("checkpoint-"):
            ...
            checkpoints = list(
                filter(lambda x: x.startswith("checkpoint-"), os.listdir(config.resume_from))
            )
    ...
    if config.prompt_fn == "general_ocr":
    ...
    elif config.prompt_fn == "alphagrpo20k":
        train_dataset = JsonlPromptDataset(config.dataset, "train")
        test_dataset = JsonlPromptDataset(config.dataset, "test")
        train_sampler = DistributedKRepeatSampler(
            dataset=train_dataset,
            batch_size=config.sample.train_batch_size,
        ...
        raise NotImplementedError(
            "Supported prompt_fn values: general_ocr, geneval, alphagrpo20k"
        )
    ...
    if config.resume_from:
        ...
        state_file = os.path.join(config.resume_from, "training_state.json")
        if os.path.exists(state_file):
            with open(state_file) as handle:
                training_state = json.load(handle)
            first_epoch = int(training_state["epoch"]) + 1
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            if epoch < config.skip_initial_epochs:
                continue
            # sample
