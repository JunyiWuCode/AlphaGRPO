...
# Public distributed launch variables. ARNOLD_* is kept only as a fallback
# for users running this script in the original internal environment.
...
if [ ${DATASET} == "mathvista-test" ]; then
    ...
    # FIXME
    python eval/vlm/eval/mathvista/extract_answer_mp.py --output_file 'results.json' "${ARGS[@]:1}"
    python eval/vlm/eval/mathvista/calculate_score.py --output_file 'results.json' "${ARGS[@]:1}" --score_file score.json
fi
