...
def generate_image_with_think(
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        output = tokenizer.batch_decode(unpacked_latent.transpose(1, 0))
        think_output = [t.split('<|im_end|>')[0].split('<|im_start|>')[1] for t in output]


    print("="*30, "original think", "="*30)
    print(think_output) 
    if simple_think:
        think_output_list = [t.split("</think>") for t in think_output]
        think_output = [t[1].strip() if len(t) > 1 and t[1] != "" else "" for t in think_output_list]
    ...
    for i, idx in enumerate(idx_list):
        ...
        if os.path.exists(short_desc_image_path):
            ...
            if args.think:
                ...
                with open(short_desc_image_path.replace('.png', '.txt'), "w") as f:
                    f.write(think_output[0])
            else:
            ...
            tmp_image_list[0].save(short_desc_image_path)
        ...
        if os.path.exists(long_desc_image_path):
            ...
            if args.think:
                ...
                with open(long_desc_image_path.replace('.png', '.txt'), "w") as f:
                    f.write(think_output[0])
            else:
                tmp_image_list = generate_image(
                    prompt=long_description,
            ...
            tmp_image_list[0].save(long_desc_image_path)
