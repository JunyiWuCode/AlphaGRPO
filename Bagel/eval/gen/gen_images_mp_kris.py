...
def setup_distributed():
    dist.init_process_group(backend="nccl", timeout=datetime.timedelta(seconds=3600000))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image-path', type=str, default="eval/gen/kris/KRIS_Bench")
    ...
    indexs = list(range(total_metadatas))[rank::world_size]
    prompts_per_gpu = len(indexs)
    print(f"GPU {rank}: Processing {prompts_per_gpu} prompts (indices {indexs[0]} to {indexs[-1]})")
    ...
    for idx in indexs:
        ...
        print(f"GPU {rank} processing prompt {indexs.index(idx)}/{prompts_per_gpu}: '{prompt}'")
