...
def setup_distributed():
    dist.init_process_group(backend="nccl", timeout=datetime.timedelta(seconds=360000))
...
def merge_images_2x2(images):
    if len(images) != 4:
        raise ValueError("必须提供 4 张图片")
    ...
    grid_img.paste(resized_images[1], (base_w, 0))
    grid_img.paste(resized_images[2], (0, base_h))
    grid_img.paste(resized_images[3], (base_w, base_h))

    return grid_img


def split_image_2x2(grid_img):
    """
    Splits a 2x2 grid image back into 4 separate images.
    Returns a list of images in the order: Top-Left, Top-Right, Bottom-Left, Bottom-Right.
    """
    total_w, total_h = grid_img.size
    
    # Calculate the width and height of a single sub-image
    w = total_w // 2
    h = total_h // 2
    
    # Crop the 4 images (left, top, right, bottom)
    # Top-Left
    img1 = grid_img.crop((0, 0, w, h))
    # Top-Right
    img2 = grid_img.crop((w, 0, total_w, h))
    # Bottom-Left
    img3 = grid_img.crop((0, h, w, total_h))
    # Bottom-Right
    img4 = grid_img.crop((w, h, total_w, total_h))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_output_dir", type=str, required=True, help="Root directory where the original source images are actually located.")
    ...
    metadatas = json.load(open(args.metadata_file, "r", encoding="utf-8"))

    # Load DPG Results Metadata
    # Format: /path/to/old/55.jpg, 0.9, 0.9
    print(f"Loading metadata from {args.metadata_file}...")
    with open(args.metadata_file, "r", encoding="utf-8") as f:
        all_lines = f.readlines()

    print(f"Loading metadata from {args.source_output_dir}...")
    with open(os.path.join(args.source_output_dir, "dpg-bench_results.txt"), "r", encoding="utf-8") as f:
        all_lines = f.readlines()
    ...
    for line in all_lines:
        ...
        parts = line.split(',')
        if len(parts) == 3:
            file2score[os.path.basename(parts[0].strip())] = float(parts[-1])
    ...
    for i, idx in enumerate(idx_list):
        ...
        filename = metadata['filename'].replace('txt', 'jpg')

        print(f"GPU {rank} processing prompt {i}/{len(idx_list)}: '{prompt}'")

        source_image_path = os.path.join(args.source_output_dir, "images", filename)
        source_image = Image.open(source_image_path).convert("RGB")

        sample_path = os.path.join(output_dir, "images", filename)
        os.makedirs(os.path.join(output_dir, "images"), exist_ok=True)
        os.makedirs(os.path.join(output_dir, "think_text"), exist_ok=True)
        ...
        if args.only_wrong and filename in file2score and file2score[filename] == 1.0:
            source_image.save(sample_path)
            with open(os.path.join(output_dir, 'think_text', metadata['filename']), "w") as f:
                f.write('Skip the reflection on this image.')
            print(f"GPU {rank} skip prompt {i}/{len(idx_list)}: '{prompt}'")
            continue

        source_images = split_image_2x2(source_image)
        ...
        image = merge_images_2x2(image_list)
        # image = image.crop(image.getbbox())
        image.save(sample_path)

        think_output = json.dumps(think_text_list, indent=2)
        with open(os.path.join(output_dir, 'think_text', metadata['filename']), "w") as f:
