...
if __name__ == "__main__":
    ...
    parser.add_argument("--metadata_file_root", type=str, required=True, help="JSONL file containing lines of metadata for each prompt.")
    ...
    metadata_files = sorted(os.listdir(args.metadata_file_root))
    metadata_files = os.listdir(args.metadata_file_root)
    print(f'Num of metafiles to run in {args.metadata_file_root}: {len(metadata_files)}')

    metadatas = []
    for metadata_file in metadata_files:
        with open(os.path.join(args.metadata_file_root, metadata_file), "r") as fp:
            metadatas += [json.loads(line) for line in fp]
    total_metadatas = len(metadatas)
    
    source_result_root = os.path.join(os.path.dirname(os.path.dirname(args.source_output_dir)), 'eval_results')
    print(source_result_root)
    idx_list = list(range(total_metadatas))[rank::world_size]
    print(f"GPU {rank}: Processing {len(idx_list)} prompts in {metadata_file}")
    for i, idx in enumerate(idx_list):
        ...
        model_name = 'bagel' # currently we fix this.
        type_dir = os.path.join(args.output_dir, data_type, model_name)
        ...
        short_desc_think_path = os.path.join(short_desc_dir, f"{idx}.txt")
        long_desc_think_path = os.path.join(long_desc_dir, f"{idx}.txt")

        source_type_dir = os.path.join(args.source_output_dir, data_type, model_name)
        source_short_desc_dir = os.path.join(source_type_dir, 'short_description')
        source_long_desc_dir = os.path.join(source_type_dir, 'long_description')
        source_short_desc_image_path = os.path.join(source_short_desc_dir, f"{idx}.png")
        source_long_desc_image_path = os.path.join(source_long_desc_dir, f"{idx}.png")

        
        source_result_dir = os.path.join(source_result_root, model_name, data_type)
        source_short_result_dir = os.path.join(source_result_dir, "short")
        source_long_result_dir = os.path.join(source_result_dir, "long")
        source_short_result = json.load(open(os.path.join(source_short_result_dir, f"{idx}.json")))
        source_long_result = json.load(open(os.path.join(source_long_result_dir, f"{idx}.json")))

        source_short_result_correct = source_short_result['gt_answers'] == source_short_result['model_pred']
        source_long_result_correct = source_long_result['gt_answers'] == source_long_result['model_pred']

        if os.path.exists(short_desc_image_path) and os.path.exists(short_desc_think_path.replace('.png', '.txt')):
            print(f"File {short_desc_image_path} already exists. Skipping...")
        else:
            print(f"GPU {rank} processing prompt {i}/{len(idx_list)}: '{short_description}'")

            source_image = Image.open(source_short_desc_image_path).convert('RGB')
            if not args.only_wrong or (args.only_wrong and not source_short_result_correct):
                ...
                tmp_image.save(short_desc_image_path)
                with open(short_desc_think_path, 'w') as f:
                    f.write(think_output)
            else:
                print(f"GPU {rank} skipping the correct generation for prompt: {short_description}")
                source_image.save(short_desc_image_path)
                with open(short_desc_think_path, 'w') as f:
                    f.write("Source Image is correct. Direct copy source image")

        if os.path.exists(long_desc_image_path) and os.path.exists(long_desc_think_path.replace('.png', '.txt')):
            print(f"File {long_desc_image_path} already exists. Skipping...")
        else:
            print(f"GPU {rank} processing prompt {i}/{len(idx_list)}: '{long_description}'")
            
            source_image = Image.open(source_long_desc_image_path)
            if not args.only_wrong or (args.only_wrong and not source_long_result_correct):
                tmp_image, think_output = reflect_image_v2(
                    images=[source_image],
                ...
                tmp_image.save(long_desc_image_path)
                with open(long_desc_think_path, 'w') as f:
                    f.write(think_output)
            else:
                print(f"GPU {rank} skipping the correct generation for prompt: {long_description}")
                source_image.save(long_desc_image_path)
