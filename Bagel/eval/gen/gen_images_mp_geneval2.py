...
if __name__ == "__main__":
    ...
    for i, idx in enumerate(idx_list):
        ...
        image_filename = f"{idx:05}.png"
        image_filepath = os.path.abspath(os.path.join(output_dir, image_filename))
        local_results_map[prompt] = image_filepath
        ...
        if args.think:
            ...
            think_text_list.extend(think_output)

            with open(os.path.join(output_dir, f"{idx:05}.txt"), "w") as f:
                f.write(think_text_list[0])   
        else:
        ...
        image_list[0].save(image_filepath)

    # Wait for all processes to finish generation
    dist.barrier()

    # Gather all dictionaries from all GPUs to Rank 0
    gathered_results = [None for _ in range(world_size)]
    dist.all_gather_object(gathered_results, local_results_map)
    if rank == 0:
        final_mapping = {}
        for res_map in gathered_results:
            final_mapping.update(res_map)
        
        # Save the final {prompt: filepath} dictionary
        json_output_path = os.path.join(output_dir, "geneval_image_map.json")
        with open(json_output_path, "w", encoding="utf-8") as f:
            json.dump(final_mapping, f, indent=4, ensure_ascii=False)
        
        print(f"Successfully saved prompt-to-filepath mapping to {json_output_path}")
