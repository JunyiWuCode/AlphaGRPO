    ...
    if start_index != -1 and end_index != -1 and start_index != end_index:
        ...
        try:
            ...
            try:
                ...
                global prompt_hash
                global cannot_fix_count
                hash_str = hash(json_str)
                if hash_str == prompt_hash:
                    cannot_fix_count += 1
                else:
                    cannot_fix_count = 0
                    prompt_hash = hash_str

                if cannot_fix_count == 3:
                    print("Failed to fix the json content in the same string. Give a random value.")
                    new_data = {'score': [random.randint(0, 8)], "reasoning": f"guess_if_cannot_parse | {json_str}"}
                    cannot_fix_count = 0
                    hash_str = None
                    return new_data
                return False
