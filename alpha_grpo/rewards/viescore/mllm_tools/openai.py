...
class GPT4v():
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=False, model_name="gpt-4-vision-preview"):
        ...
        self.url = os.environ.get('OPENAI_API_URL', "https://api.openai.com/v1/chat/completions")
...
class Doubao(GPT4v):
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=False, model_name="YOUR_MODEL_ENDPOINT"):
        super().__init__(api_key_path, are_images_encoded, model_name)
        self.url = os.environ.get('OPENAI_API_URL', "https://api.openai.com/v1/chat/completions")
        self.model_name = model_name
        self.use_encode = are_images_encoded

        self.model_names = ["YOUR_MODEL_NAME_HERE"]
        self.api_keys = ["YOUR_API_KEY_HERE"]
...
class Qwen3VL(GPT4v):
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=True):
        super().__init__(api_key_path, are_images_encoded)
        self.api_key = "EMPTY"
        
        model_name = os.environ.get('OPENAI_MODEL_NAME', "Qwen/Qwen3-VL-235B-A22B-Thinking")
        self.url = os.environ.get('OPENAI_API_URL', "https://api.openai.com/v1/chat/completions")
        print(self.url)
        self.model_name = model_name
        self.use_encode = are_images_encoded
    ...
    def get_parsed_output(self, prompt):
        for _ in range(2):
            try:
                ...
                if "</think>" in results:  # we find the </think> tag might be in the answer. So we remove this manully.
